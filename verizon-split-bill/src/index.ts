...
import { QuoteOfTheDay, Verizon } from "../types";
...
async function main() {
  try {
    ...
    let summaryResponse: Verizon.Summary | null = null;
    let detailsResponse: Verizon.Details | null = null;
      ...
      if (url.includes("/summary") || url.includes("/details")) {
        ...
        if (response?.ok()) {
          ...
          if (url.includes("/summary")) {
            summaryResponse = json as Verizon.Summary;
          } else if (url.includes("/details")) {
            detailsResponse = json as Verizon.Details;
          }
        }
      }
    });
...
async function fetchQuoteOfTheDay(): Promise<QuoteOfTheDay> {
